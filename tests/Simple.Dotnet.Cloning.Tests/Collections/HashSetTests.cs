...
namespace Simple.Dotnet.Cloning.Tests.Collections
{
    public class HashSetTests
    {
        ...
        public void HashSet_DeepClone_Should_Have_Cloned_Values()
        {
            ...
            IsEqual(collection, new Wrapper<HashSet<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, new WrapperRecord<HashSet<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, new WrapperStruct<HashSet<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, new WrapperReadonly<HashSet<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
        }
        ...
        [Fact]
        public void HashSet_ShallowClone_Should_Be_Different_As_Interface()
        {
            var collection = Create<HashSetTests>(10, i => new());

            IsEqual(collection, (HashSet<HashSetTests>)((ISet<HashSetTests>)collection).DeepClone(), (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, (HashSet<HashSetTests>)((IEnumerable<HashSetTests>)collection).DeepClone(), (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, (HashSet<HashSetTests>)((ICollection<HashSetTests>)collection).DeepClone(), (f, s) => f != s).Should().BeTrue();
        }
        ...
        {
            var collection = Create<HashSetTests>(10, i => new());
            IsEqual(collection, (HashSet<HashSetTests>)new Wrapper<IEnumerable<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, (HashSet<HashSetTests>)new WrapperRecord<IEnumerable<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, (HashSet<HashSetTests>)new WrapperStruct<IEnumerable<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
            IsEqual(collection, (HashSet<HashSetTests>)new WrapperReadonly<IEnumerable<HashSetTests>>(collection).DeepClone().Value, (f, s) => f != s).Should().BeTrue();
        }
        ...
        static bool IsEqual<T>(HashSet<T> collection, HashSet<T> clone, Func<T, T, bool> comparer)
        {
